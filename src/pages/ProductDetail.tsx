...
const ProductDetail = () => {
  ...
  const { data: relatedProducts, isLoading: isLoadingRelated } = useQuery({
    ...
    queryFn: () =>
      productApi.getByCategory(product?.category_id || 1, 4),
  ...
  const getProductImages = (p: Product) => {
    ...
    if (p.images) {
      try {
        ...
        const imagesStr = typeof p.images === 'string' ? p.images : JSON.stringify(p.images);
    ...
    return (
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        >
          ...
          {currentLanguage === "fr" ? "Retour aux produits" : "العودة إلى المنتجات"}
        </button>
      </div>
    );
  }
  ...
  try {
    const featuresField = currentLanguage === "fr" ? product.features_fr : product.features_ar;
    if (featuresField) {
